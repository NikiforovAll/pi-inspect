...
const SNAPSHOT_DIR = process.env.INSPECT_SNAPSHOT_DIR || path.join(STATE_DIR, 'snapshots');
...
async function writeIndex(sessions) {
  await fsp.mkdir(SNAPSHOT_DIR, { recursive: true, mode: 0o700 });
  await fsp.chmod(SNAPSHOT_DIR, 0o700);
  await fsp.writeFile(INDEX_PATH, JSON.stringify({ sessions }, null, 2), { encoding: 'utf8', mode: 0o600 });
  await fsp.chmod(INDEX_PATH, 0o600);
}
...
async function readLatestSnapshot() {
  ...
  for (const session of sorted) {
    const snapshot = await readSnapshot(session.id);
    if (snapshot) {
      kept.push(session);
      if (!latest) latest = snapshot;
    }
  }
  if (kept.length !== sessions.length) await writeIndex(kept);
  return latest;
}
